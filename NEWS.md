...
* `SecureSession` R6 class for persistent sandboxed R sessions with tool-call IPC
  over Unix domain sockets.
* `SecureSessionPool` for pre-warmed session pools with automatic dead-session recovery.
* `execute_r()` convenience function for one-shot sandboxed execution.
* `securer_tool()` for defining typed tool functions with runtime argument validation.
* `validate_code()` for pre-execution syntax checking and dangerous pattern detection.
* `default_limits()` for inspecting and customizing resource limit defaults.

## Session Ergonomics

* `$restart()` method to explicitly restart a dead or stuck child process.
* `$tools()` accessor to inspect registered tools and their argument specs.
* `print()`/`format()` methods for `SecureSession` and `SecureSessionPool`.
* `sandbox_strict` parameter to error when sandbox tools are unavailable instead
  of silently falling back to unsandboxed execution.

## Sandbox Support

* **macOS**: Full OS-level sandbox via Seatbelt (`sandbox-exec`) - filesystem writes
  blocked, network denied, reads restricted to R libraries.
* **Linux**: Full namespace isolation via bubblewrap (`bwrap`) - read-only mounts,
  network namespace isolation, `/tmp`-only writes.
* **Windows**: Environment variable isolation (clean HOME, empty R_LIBS_USER) plus
  Job Object resource limits (memory, CPU time, process count) via PowerShell.

## Resource Controls

* `ulimit`-based resource limits on Unix: CPU time, memory, file size, process count,
...
* Concurrent execution guard preventing parallel `$execute()` on the same session.
