...
If you prefer not to manage `$close()` manually, use `with_secure_session()`.
It creates a session, passes it to your function, and guarantees cleanup:
...
`SecureSession` includes several features for hardening LLM agent deployments:

- **`max_code_length`** (in `$execute()`): Reject code strings longer than a
  threshold (default 100,000 characters) before parsing.
- **`max_executions`** (in `$new()`): Limit the total number of `$execute()`
  calls on a session. Once reached, further calls error immediately.
- **`pre_execute_hook`** (in `$new()`): A function called before every
  execution. Return `FALSE` to block it. Useful for content-based filtering.
- **`sanitize_errors`** (in `$new()`): Strip file paths, PIDs, and hostnames
...
Each `$execute()` call accepts a `timeout` in seconds (default 30). If exceeded,
the child is killed and the session auto-recovers for subsequent calls:
