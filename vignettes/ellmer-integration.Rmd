...
`securer_as_ellmer_tool()` connects the two packages. It wraps a
...
The LLM receives a tool called `execute_r_code` that accepts a
...
See `vignette("deployment")` for details on sandbox modes, resource limits,
and timeouts.
