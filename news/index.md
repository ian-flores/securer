...
- **`docker-spawn` sandbox backend.** Spawns the child R process inside
  a fresh `docker run` container per session, with `--network=none`,
  memory and CPU caps, and a bind-mount of the UDS socket directory. Opt
  in via `SECURER_SANDBOX_MODE=docker-spawn`; gated at runtime on a
  reachable docker daemon. Image is configurable via
  `SECURER_DOCKER_IMAGE` (default `rocker/r-base:latest`). See
  `inst/docker/Dockerfile` for a reference image.
