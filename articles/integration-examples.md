...
### Where securer fits
     ...
     Shiny / Plumber / Batch script
    +------------------------------+
    |  Application layer           |     User requests or code snippets
    |  (UI, routes, lapply)        |     arrive here.
    +---------|--------------------+
              |
              v
    +------------------------------+
    |  securer                     |     SecureSession or SecureSessionPool
    |  - Sandbox (Seatbelt/bwrap) |     manages child R processes with
    |  - IPC (Unix domain socket)  |     OS-level isolation.
    |  - Resource limits (ulimit)  |
    +---------|--------------------+
              |
              v
    +------------------------------+
    |  Sandboxed R child           |     Code executes here. Tool calls
    |  - eval(user code)           |     pause execution and are fulfilled
    |  - tool call -> host -> back |     by the host via IPC.
    +------------------------------+
